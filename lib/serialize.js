...
class BorshSerializer {
    ...
    encode(value, schema) {
        if (typeof schema === 'string') {
            if (types_1.numbers.includes(schema))
                this.encode_integer(value, schema);
            if (schema === 'string')
                this.encode_string(value);
            if (schema === 'bool')
                this.encode_boolean(value);
        }
        if (typeof schema === 'object') {
            if ('option' in schema)
                this.encode_option(value, schema);
            if ('array' in schema)
                this.encode_array(value, schema);
            if ('set' in schema)
                this.encode_set(value, schema);
            if ('map' in schema)
                this.encode_map(value, schema);
            if ('struct' in schema)
                this.encode_struct(value, schema);
        }
    ...
    encode_array(value, schema) {
        ...
        if (utils.isArrayLike(value))
            return this.encode_arraylike(value, _schema);
        if (value instanceof ArrayBuffer)
            return this.encode_buffer(value, _schema);
